...
const addListing = async (vehicle, formInput) => {
  try {
    console.log(
    ...
    const listingCollection = collection(db, DbConstants.COLLECTION_LISTINGS);

    const coordinates = await doForwardGeocode(formInput.pickupAddress);
    const postalAddress = await doReverseGeocode(coordinates);
